...
export default function AdminInventoryPage() {
  ...
  const openAdd = () => {
    setEditingProduct({ name: '', category: 'food', petType: 'dog', brand: '', price: 0, image: 'https://images.unsplash.com/photo-1589924691995-400dc9ecc119?w=600&h=600&fit=crop', images: [], description: '', features: [], weight: '', featured: false, inStock: true, stockCount: 0, sku: '' });
    setShowModal(true);
  };

  const openEdit = (product: ApiProduct) => { setEditingProduct({ ...product }); setShowModal(true); };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <button onClick={fetchProducts} className="flex items-center gap-2 px-3 py-2.5 border border-gray-200 text-gray-600 font-medium rounded-xl hover:bg-gray-50 transition-colors text-sm"><RefreshCw className="w-4 h-4" /></button>
          <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2.5 bg-indigo-600 text-white font-medium rounded-xl hover:bg-indigo-700 transition-colors text-sm"><Plus className="w-4 h-4" /> Add Product</button>
        </div>
      </div>

      <div className="relative mb-6 max-w-md">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search products, brands, SKU..." className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-gray-500 border-b border-gray-100 bg-gray-50">
                  <th className="px-6 py-3 font-medium cursor-pointer hover:text-gray-900" onClick={() => handleSort('name')}>Product{arrow('name')}</th>
                  <th className="px-6 py-3 font-medium cursor-pointer hover:text-gray-900" onClick={() => handleSort('category')}>Category{arrow('category')}</th>
                  <th className="px-6 py-3 font-medium">Brand</th>
                  <th className="px-6 py-3 font-medium cursor-pointer hover:text-gray-900" onClick={() => handleSort('price')}>Price{arrow('price')}</th>
              ...
              <tbody>
                {filtered.map(product => (
                  <tr key={product.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-3">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${product.inStock && product.stockCount > 10 ? 'bg-green-100 text-green-700' : product.inStock && product.stockCount > 0 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>
                        {product.inStock ? `${product.stockCount} units` : 'Out of Stock'}
                      </span>
                    </td>
                    <td className="px-6 py-3">
                      <div className="flex items-center gap-2">
                        <button onClick={() => openEdit(product)} className="p-1.5 rounded-lg hover:bg-indigo-50 text-gray-400 hover:text-indigo-600 transition-colors"><Pencil className="w-4 h-4" /></button>
                        {deleteConfirm === product.id ? (
                          <div className="flex items-center gap-1">
                            <button onClick={() => handleDelete(product.id)} disabled={deleting === product.id} className="p-1.5 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 disabled:opacity-50">
                              {deleting === product.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                            </button>
                            <button onClick={() => setDeleteConfirm(null)} className="p-1.5 rounded-lg bg-gray-50 text-gray-400 hover:bg-gray-100"><X className="w-4 h-4" /></button>
                          </div>
                        ) : (
                          <button onClick={() => setDeleteConfirm(product.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors"><Trash2 className="w-4 h-4" /></button>
                        )}
                      </div>
                    </td>
                  </tr>
                ))}
                {filtered.length === 0 && <tr><td colSpan={6} className="px-6 py-10 text-center text-gray-400">No products found.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
      ...
      {showModal && editingProduct && (
        <ProductModal product={editingProduct} isNew={!editingProduct.id} onSave={handleSave} onClose={() => { setShowModal(false); setEditingProduct(null); }} />
      )}
    </div>
  );
}
...
function ProductModal({ product, isNew, onSave, onClose }: { product: Partial<ApiProduct>; isNew: boolean; onSave: (data: Record<string, any>, isNew: boolean) => Promise<void>; onClose: () => void }) {
  const [form, setForm] = useState({
    id: product.id || '', name: product.name || '', category: (product.category || 'food').toUpperCase(), petType: (product.petType || 'dog').toUpperCase(),
    brand: product.brand || '', price: product.price || 0, originalPrice: product.originalPrice || undefined,
    image: product.image || 'https://images.unsplash.com/photo-1589924691995-400dc9ecc119?w=600&h=600&fit=crop', images: product.images || [],
    description: product.description || '', features: product.features || [], weight: product.weight || '', dimensions: product.dimensions || '',
    featured: product.featured || false, stockCount: product.stockCount || 0, sku: product.sku || '',
  });
  const [featuresText, setFeaturesText] = useState((product.features || []).join('\n'));
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');

  const handleAutoSku = () => { if (isNew && !form.sku) setForm(f => ({ ...f, sku: generateSku(f.brand, f.category) })); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data: Record<string, any> = {
        name: form.name.trim(), category: form.category, petType: form.petType, brand: form.brand.trim(),
        price: Number(form.price), image: form.image.trim(), images: form.images.length > 0 ? form.images : [form.image.trim()],
        description: form.description.trim(), features: featuresText.split('\n').map(f => f.trim()).filter(Boolean),
        weight: form.weight.trim(), featured: form.featured, stockCount: Number(form.stockCount), sku: form.sku.trim(),
      };
      if (form.originalPrice && Number(form.originalPrice) > 0) data.originalPrice = Number(form.originalPrice);
      if (form.dimensions && form.dimensions.trim()) data.dimensions = form.dimensions.trim();
      if (!isNew) data.id = form.id;
      await onSave(data, isNew);
    } catch (err: any) {
      let msg = err.message || 'Failed to save product.';
      if (msg === 'Validation failed.' && err.errors) msg = err.errors.map((e: any) => `${e.field}: ${e.message}`).join(', ');
      setError(msg);
    } finally {
      setSaving(false);
    }
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm bg-white">
                {categoryOptions.map(c => <option key={c} value={c}>{c.charAt(0) + c.slice(1).toLowerCase()}</option>)}
              </select>
            </div>
            <div>
              ...
              <select value={form.petType} onChange={e => setForm({ ...form, petType: e.target.value })} className="w-full px-3 py-2 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm bg-white">
                {petTypeOptions.map(p => <option key={p} value={p}>{p === 'ALL' ? 'All Pets' : p.charAt(0) + p.slice(1).toLowerCase()}</option>)}
              </select>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="flex gap-1.5">
                ...
                {isNew && <button type="button" onClick={() => setForm(f => ({ ...f, sku: generateSku(f.brand, f.category) }))} className="px-2.5 py-2 border border-gray-200 rounded-xl text-xs text-gray-500 hover:bg-gray-50 whitespace-nowrap">Auto</button>}
              </div>
            </div>
            <div>
          ...
          <div>
            ...
            {form.image && <img src={form.image} alt="Preview" className="mt-2 w-20 h-20 rounded-lg object-cover border border-gray-200" onError={e => (e.currentTarget.style.display = 'none')} onLoad={e => (e.currentTarget.style.display = 'block')} />}
          </div>
          <div>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 py-2.5 border border-gray-200 rounded-xl text-gray-600 font-medium hover:bg-gray-50 transition-colors text-sm">Cancel</button>
            <button type="submit" disabled={saving} className="flex-1 py-2.5 bg-indigo-600 text-white font-medium rounded-xl hover:bg-indigo-700 transition-colors text-sm disabled:opacity-60 flex items-center justify-center gap-2">
